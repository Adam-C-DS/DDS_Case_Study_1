...
BeerBrewery %>% group_by(IBUFac) %>% filter(IBU != "NA") %>% summarise(countibu = n()) %>%
  arrange(-countibu) %>% slice(1:15)  %>% ggplot(aes(x = IBUFac, y = countibu)) +
  geom_col(width = 0.75) + xlab("IBU Factor") + ylab("# of Beers with this IBU") + 
...
BeerBrewery %>% group_by(IBUFac) %>% summarise(countibu = n()) %>% arrange(-countibu) %>% slice(1:15) %>%
  ggplot(aes(x = IBUFac, y = countibu)) + geom_col(width = 0.75) + xlab("IBU Factor") + ylab("# of Beers with this IBU") +
  theme(axis.text.x = element_text(angle = 90, vjust = -0.05))
```
