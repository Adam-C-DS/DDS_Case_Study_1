...
```{r}
# Not part of a specific question - IBU Distribution Characteristics - Summary Stats and distribution of IBU before impute ----
...
Myimpute <- preProcess(BeerBrewery1[,4:5], method = "knnImpute", k = 20)
Myimpute <- predict(Myimpute, BeerBrewery1)

# Unscales the IBU So we can analyze it
Myimpute = Myimpute %>% mutate(IBUPred = (IBU*sd(BeerBrewery1$IBU, na.rm = TRUE) + mean(BeerBrewery1$IBU, na.rm = TRUE)))
```

```{r, warning=FALSE}
# Comparing Imputed IBUs with observed data (q3) ----
p1 = BeerBreweryPredicted %>% ggplot(aes(x = IBU)) + geom_histogram(binwidth = 3)
p2 = Myimpute %>% ggplot(aes(x = IBUPred)) + geom_histogram(binwidth = 3)
p3 = BeerBreweryNA.delete %>% ggplot(aes(x = IBU)) + geom_histogram(binwidth = 3)
plot_grid(p1,p2,p3, labels = c("LM","KNN", "OBS"), ncol = 3)
```
...
Part 8 still needs the classifier - this should be a near copy of asynch material
```{r}
# Part of question 8 - Creating the IPA/Ale data set from the linear predicted data set ----
