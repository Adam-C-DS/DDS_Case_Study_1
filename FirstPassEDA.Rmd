...
```{r}
linearmodel1 <- lm(IBU ~ ABV, data = BeerBreweryNA.delete)
...
```{r}
linearmodel2 <- lm(log(IBU) ~ poly(log(ABV),3), data = BeerBreweryNA.delete)
...
```{r}
linearmodel3 <- lm(scale(IBU) ~ scale(ABV), data = BeerBreweryNA.delete)
