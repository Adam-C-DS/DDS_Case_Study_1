...
map2 = merge(statepop, mapBreweries, by.x = "abbr", by.y = "State")
plot_usmap(data = map2, values = "StateMedian", color = "red", labels = TRUE, label_color = "white") + 
  scale_fill_continuous(name = "State Median IBU", label = scales::comma) + 
...
```{r}
# Part of question 8 - Find best k value (5 apparently) ----
# this does the same as above but averages over many runs to get a better view of the true accuracy of the k value
iterations = 50
numks = 40
...
for(i in 1:numks)
{
  classifications = knn(train[,4:5],test[,4:5],train$IPAorAle, prob = TRUE, k = i)
...
classifications = knn(train[,4:5],test[,4:5],train$IPAorAle, prob = TRUE, k = 5)
table(classifications,test$IPAorAle)
...
# We can then use the most popular ABV and IBU to design a custom type if possible.
# Do we want to bring in outside data here from some reputable sources to back our beer choice?
