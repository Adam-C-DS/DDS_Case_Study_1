...
# Creates ABV Factor based on summary of the column: Weak - Abv(0<0.056), Normal - Abv(0.056<= 0.067), Strong - Abv(>0.067)
...
BeerBreweryNA.predict = BeerBrewery %>% filter(is.na(IBU))
```
...
# The next set shows the same thing but with only the top 15 -  
## I think this may get the point across better. I think we should lead with the full set of heres everything just to get a feel for how big this stuff really
## is - then switch to the top x - that way side by side comparisons are easier and we have reference beers/styles/IBUs/ to talk about.
...
```{r}
# Looking at ABV by factor - do these breaks work? need adjusting? Most people prefer normal and more prefer strong to weak.
BeerBrewery %>% group_by(ABVFac) %>% summarise(CountABVFac = n()) %>% na.omit() %>%
  ggplot(aes(x = ABVFac, y = CountABVFac)) + geom_col(width = 0.75) + geom_text(aes(label = CountABVFac), vjust = -0.5, size = 4) + 
...
g2 = BeerBrewery %>% ggplot(aes(x = ABV)) + geom_boxplot() + xlim(0,0.125) + theme(axis.ticks.y = element_blank())#, axis.text.y = element_blank())
...
```{r}
# I'll have to check this out later - not working at the moment
g1 = BeerBreweryNA.delete %>% ggplot(aes(x = IBU)) + geom_histogram(binwidth = 2) + xlim(0,150) +
  theme(axis.title.x = element_blank(), axis.title.y = element_blank())

g2 = BeerBreweryNA.delete %>% ggplot(aes(x = IBU)) + geom_boxplot() + xlim(0,150) + theme(axis.ticks.y = element_blank())#, axis.text.y = element_blank())

g3 = BeerBreweryNA.delete %>% ggplot(aes(sample = IBU)) + stat_qq_line() + stat_qq() + ylim(0,150) + xlab("Quantile") + ylab("IBU")

g4 = ggdraw() + draw_label("Distribution Characteristics \nof IBU", color = "Gray2", size = 20)
...
# Linear Models 1-3: Looking at how well a linear model will predict/impute the data. 
```{r}
# Very Simple
...
```{r}
# Very Simple with a log
linearmodel1.log <- lm(log(IBU) ~ ABV, data = train)
summary(linearmodel1.log)
ols_plot_resid_fit(linearmodel1.log)
ols_plot_resid_lev(linearmodel1.log)
ols_plot_resid_qq(linearmodel1.log)
ols_plot_resid_hist(linearmodel1.log)
ols_plot_cooksd_bar(linearmodel1.log)

```

```{r}
# Pretty Complex - came up with this playing around. there is some curvature present in the Resdiual Normal QQ - however 
...
```{r}
# Medium Complexity
linearmodel3 <- lm(log(IBU) ~ poly(log(ABV,3)), data = train)
...
There is some serious correlation in the above residual plots which is a bit troubling. This would suggest we need a correction of some sort or more predictors (of which we have none). Logging the response does not help (model 1 v logmodel1 and model 2 v model 3). Model 2 gets rid of a lot of the correlation - but adds a non-constant variance violation, which again is not alleviated by log transforms.

All this leads me to think we may want a non-parametric model. Thus the Naive Bayes (Though the conditional independence may be violated). I think this will have the advantage over KNN for 2 reasons: 

1) We will want to try non-numeric variable types. 
2) KNN works best with scaled numeric variables - the scaling, as well as the KNN, creates issues with interpretation - this doesn't completly exclude it but maybe works best as a last resort.
...
#Are the fits shown here a bit misleading compared to their equations?
...
```
# NaiveBayes Prediction - This is apparently more difficult than anticipated, messed with this for awhile. - Checking KNN
```{r}
# This is currently broke I think
# predict(NBmodel, newdata, type = raw) / type = raw for probabilities
# naiveBayes(predictors, response, laplace)
NB.Model.1 <- naiveBayes(train[,4], train$IBUFac)
predict(NB.Model.1, data.frame(ABV = 0.094), type = "raw")
...
```{r}
# Uses linear model 1 to predict IBU from ABV -  need to break the predictions out of the KNN next
# This predicts right into BeerBrewery we may need to offload these into a second data set depending on analysis needs.
BeerBreweryPredicted <- BeerBrewery %>% mutate(Prediction = predict(linearmodel2, .)) %>% mutate(IBU = ifelse(is.na(IBU), Prediction, IBU))

```


```{r, warning=FALSE}
BeerBreweryPredicted %>% group_by(State) %>% na.omit() %>% summarise(StateMedian = median(ABV)) %>% arrange(-StateMedian) %>% slice(1:15) %>%
...
# Perfect! This looks amazing!
mapBreweries = BeerBreweryPredicted %>% na.omit() %>% group_by(State) %>% summarise(StateMedian = median(ABV))
mapBreweries$State = as.character(mapBreweries$State)
mapBreweries$State = sub(" ", "", mapBreweries$State)
map2 = merge(statepop, mapBreweries, by.x = "abbr", by.y = "State")
...
```{r, warning=FALSE}
BeerBreweryPredicted %>% group_by(State) %>% na.omit() %>% summarise(StateMedian = median(IBU)) %>% arrange(-StateMedian) %>% slice(1:15) %>%
  ggplot(aes(x = reorder(State, -StateMedian), y = StateMedian)) + geom_col() +
  theme(axis.text.x = element_text(angle = 90, vjust = -0.05)) +  xlab("State") + ylab('Median') + 
  ggtitle("Median IBU By State (Highest)") + geom_text(aes(label = StateMedian), vjust = -0.5, size = 3)

# Perfect! This looks amazing!
mapBreweries = BeerBreweryPredicted %>% na.omit() %>% group_by(State) %>% summarise(StateMedian = median(IBU))
mapBreweries$State = as.character(mapBreweries$State)
mapBreweries$State = sub(" ", "", mapBreweries$State)
map2 = merge(statepop, mapBreweries, by.x = "abbr", by.y = "State")
plot_usmap(data = map2, values = "StateMedian", color = "red", labels = TRUE, label_color = "white") + 
...
```{r}
BeerBreweryPredicted = BeerBreweryPredicted %>% mutate(StyleChar = as.character(Style))
IPA = BeerBreweryPredicted %>% filter(str_detect(StyleChar,"IPA"))
Ale = BeerBreweryPredicted %>% filter(str_detect(StyleChar,"Ale")) %>% filter(!str_detect(StyleChar,"India Pale Ale"))
...
```{r}
sprintf("The Max ABV is %s, from the beer %s,in State %s",
        BeerBreweryPredicted$ABV[which.max(BeerBreweryPredicted$ABV)],
        BeerBreweryPredicted$Beer[which.max(BeerBreweryPredicted$ABV)],
        BeerBreweryPredicted$State[which.max(BeerBreweryPredicted$ABV)])

sprintf("The Max IBU is %s, from the beer %s,in State %s",
        BeerBreweryPredicted$IBU[which.max(BeerBreweryPredicted$IBU)],
        BeerBreweryPredicted$Beer[which.max(BeerBreweryPredicted$IBU)],
        BeerBreweryPredicted$State[which.max(BeerBreweryPredicted$IBU)])
```
