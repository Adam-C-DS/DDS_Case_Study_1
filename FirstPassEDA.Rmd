...
ABVFac = cut(BeerBrewery$ABV, breaks = c(0,0.056,0.067,0.15), labels = c("Weak ( < 0.056)", "Normal (0.056 - 0.067)", "Strong (0.067 <)"))
...
```{r}
# Looking at ABV by factor - do these breaks work? need adjusting?
BeerBrewery %>% group_by(ABVFac) %>% summarise(CountABVFac = n()) %>% 
  ggplot(aes(x = reorder(ABVFac, -CountABVFac), y = CountABVFac)) + geom_col(width = 0.75) + geom_text(aes(label = CountABVFac), vjust = -0.5, size = 4) + 
...
BeerBrewery <- BeerBrewery %>% mutate(Prediction = predict(linearmodel1, .)) %>% mutate(IBU = ifelse(is.na(IBU), Prediction, IBU))
